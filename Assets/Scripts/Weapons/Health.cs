...
public class Health : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        if (scorchMarkPrefab)
        {
            scorchMark = Instantiate(scorchMarkPrefab, Vector3.zero, Quaternion.identity);
        ...
        */
        ...
        if (health <= 0)
        {
            ...
            if (scorchMark)
            {
                ...
                Vector3 scorchPosition =
                    GetComponent<Collider>().ClosestPointOnBounds(transform.position - Vector3.up * 100);
                scorchMark.transform.position = scorchPosition + Vector3.up * 0.1f;
                scorchMark.transform.eulerAngles = new Vector3(scorchMark.transform.eulerAngles.x,
                    Random.Range(0.0f, 90.0f), scorchMark.transform.eulerAngles.z); //y值随机0到90
            }
        }
    }
