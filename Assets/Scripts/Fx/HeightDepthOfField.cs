...
public class HeightDepthOfField : MonoBehaviour
{
    ...
    void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        ...
        Vector3 toRight = GetComponent<Camera>().transform.right * cameraNear * Mathf.Tan(fovWHalf * Mathf.Deg2Rad) *
                          cameraAspect;
        ...
        dofMaterial.SetVector("_ObjectFocusParameter", new Vector4(
            t.position.y - 0.25f, t.localScale.y * 1.0f / smoothness, 1.0f,
            objectFocus ? objectFocus.GetComponent<Collider>().bounds.extents.y * 0.75f : 0.55f));
        ...
        RenderTexture lowTexture =
            RenderTexture.GetTemporary(source.width / (divider * 2), source.height / (divider * 2), 0);
