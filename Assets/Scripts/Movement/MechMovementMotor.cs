...
public class MechMovementMotor : MovementMotor
{
    ...
    void FixedUpdate()
    {
        ...
        if (adjustedMovementDirection != Vector3.zero)
            ...
            rotationAngle = 0;
        Vector3 targetAngularVelocity = Vector3.up *
                                        Mathf.Clamp(rotationAngle, -turningSpeed * Mathf.Deg2Rad,
                                            turningSpeed * Mathf.Deg2Rad);
        GetComponent<Rigidbody>().angularVelocity = Vector3.MoveTowards(GetComponent<Rigidbody>().angularVelocity,
            targetAngularVelocity, Time.deltaTime * turningSpeed * Mathf.Deg2Rad * 3); //施加旋转的力
        ...
        GetComponent<Rigidbody>().velocity = Vector3.MoveTowards(GetComponent<Rigidbody>().velocity, targetVelocity,
            Time.deltaTime * walkingSpeed * 3); // Vector3.MoveTowards : u3d api, 移向,当前的地点移向目标。
