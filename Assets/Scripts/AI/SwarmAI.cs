...
public class SwarmAI : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        for (int i = 0; i < buzzers.Length; i++)
        {
            ...
            buzzer.chargeMaterial = buzzer.transform.Find("buzzer_bot/electric_buzzer_plasma").GetComponent<Renderer>()
                .material;
    ...
    void UpdateAttack()
    {
        ...
        for (int i = 0; i < count; i++)
        {
            ...
            if (i == attackerIndex)
            {
                ...
                else
                {
                    ...
                    if (playerDist < zapDist + slowDownDist)
                    {
                        ...
                        {
                            // Zap player here
                            StartCoroutine(DoElectricArc(buzz[i]));
            ...
            else
            {
                ...
                Vector3 posSide = player.position + Vector3.Project(-playerDir * playerDist, attackerDir) +
                                  fleeDir * s * rechargeDist;
    ...
    IEnumerator DoElectricArc(BuzzerData buzz)
    {
        ...
        while (Time.time < stopTime)
        {
            ...
            buzz.electricArc.sharedMaterial.mainTextureOffset =
                new Vector2(Random.value, buzz.electricArc.sharedMaterial.mainTextureOffset.y);
            yield return 0;
        }
