...
public class SpiderAttackMoveController : MonoBehaviour
{
    ...
    {
        inRange = false;
        nextRaycastTime = lastRaycastSuccessfulTime = proximityLevel = lastBlinkTime = noticeTime = 0;
            ...
            blinkComponents =
                transform.parent
                    .GetComponentsInChildren<
                        SelfIlluminationBlink>(); // 将怪子物体身上所有绑有SelfIlluminationBlink脚本的物体的SelfIlluminationBlink放到数组中
    }
    ...
    void Update()
    {
        ...
        if ((playerDist < proximityDistance && Time.time < lastRaycastSuccessfulTime + 1) ||
            proximityLevel > proximityOfNoReturn)
    ...
    void Explode()
    {
        ...
        player.GetComponent<Rigidbody>()
            .AddExplosionForce(10, character.position, damageRadius, 0.0f, ForceMode.Impulse);
